import { Product } from "@/types/product";

export const products: Product[] = [

  {
    id: "Asad-lattafa",
    name: "Asad Lattafa",
    price: 220.0,
    originalPrice: 275.0,
    olfactoryFamily: "Âmbar Especiado",
    gender: "Masculino",
    image: "/images/products/asad-lattafa.jpeg",
    images: ["/images/products/asad-lattafa.jpeg", "/images/products/asad-lattafa-tabela.jpeg"],
    description:
      `O perfume árabe masculino que transmite intensidade, sofisticação e presença marcante.

O Asad by Lattafa da Lattafa foi criado para homens que gostam de fragrâncias poderosas, elegantes e extremamente envolventes. Sua composição combina especiarias quentes, madeiras nobres e acordes ambarados, criando uma assinatura olfativa intensa e inesquecível.

Com excelente projeção e fixação prolongada, é aquele perfume que transmite confiança, imponência e deixa presença marcada em qualquer ambiente. ✨

Ideal para quem valoriza perfumes masculinos premium com performance acima da média.

🔥 Família Olfativa

Âmbar Especiado

✨ Notas em destaque

Notas de Topo
* Pimenta Preta
* Tabaco
* Abacaxi

Notas de Corpo
* Patchouli
* Café
* Íris

Notas de Fundo
* Baunilha
* Âmbar
* Madeira Seca
* Benjoim

💼 Ideal para quem busca

✔️ Perfumes masculinos intensos e sofisticados
✔️ Alta projeção e fixação prolongada
✔️ Uso noturno e ocasiões especiais
✔️ Elegância e presença marcante
✔️ Fragrâncias extremamente elogiadas

🛍️ Disponível em

🖤 Frasco Original 100ml
✨ R$ 220,00

🖤 Decant 10ml
✨ R$ 39,90

🖤 Kit Descoberta
✨ 3 Decants de 10ml por R$ 109,90

Perfeito para conhecer a fragrância antes de investir no frasco completo ou experimentar diferentes perfumes da coleção.

Flor Árabe Perfumaria
Onde cada fragrância conta uma história. ✨`,
  },
  
 
  {
    id: "Mayar-lattafa",
    name: "Mayar Lattafa",
    price: 219.90,
    originalPrice: 274.88,
    olfactoryFamily: "Floral Frutado",
    gender: "Feminino",
    image: "/images/products/mayar-lattafa-normal.jpg",
    images: ["/images/products/mayar-lattafa-normal.jpg", "/images/products/mayar-lattafa-normal-tabela.jpeg"],
    description:
      `O perfume árabe feminino que conquista mulheres que amam fragrâncias sofisticadas, delicadas e extremamente elegantes.

Moderno, feminino e envolvente, o Mayar é perfeito para quem busca um perfume marcante na medida certa, combinando frescor frutado, flores delicadas e um toque cremoso extremamente confortável.

Sua combinação de frutas vermelhas suculentas, notas florais sofisticadas, musk e acordes aveludados cria uma fragrância luminosa, elegante e extremamente agradável, perfeita para destacar sua feminilidade com sofisticação.

É aquele perfume que transmite delicadeza, luxo e deixa um rastro memorável por onde passa. ✨

🌷 Família Olfativa

Floral Frutado

✨ Notas em destaque

Frutas vermelhas suculentas
Lichia delicada
Flores sofisticadas
Musk confortável
Acordes cremosos e elegantes

💖 Ideal para

✔️ Uso diário
✔️ Trabalho
✔️ Passeios e encontros especiais
✔️ Mulheres que amam perfumes femininos sofisticados
✔️ Quem busca fragrâncias modernas e muito elogiadas

🛍️ Disponível em:

🌸 Frasco Original 100ml
💗 R$ 219,90

🌸 Decant 10ml
✨ R$ 39,90

🌸 Kit Descoberta
✨ 3 Decants de 10ml por R$ 109,90

Perfeito para conhecer a fragrância antes de investir no frasco completo ou experimentar diferentes perfumes da coleção.
🌸 Descubra por que o Mayar é um dos perfumes árabes femininos mais elegantes e desejados do momento.

Flor Árabe Perfumaria
Onde cada fragrância conta uma história. ✨`,
  },
  {
    id: "Mayar-Natural-Intense-Lattafa",
    name: "Mayar Lattafa Natural Intense",
    price: 219.90,
    originalPrice: 274.88,
    olfactoryFamily: "Floral Aquático Frutado",
    gender: "Feminino",
    image: "/images/products/mayar-lattafa.jpg",
    images: ["/images/products/mayar-lattafa.jpg", "/images/products/mayar-lattafa-tabela.jpeg"],
    description:
      `O perfume árabe feminino que conquista mulheres que amam fragrâncias refrescantes, sofisticadas e naturalmente elegantes.

Moderno, vibrante e extremamente envolvente, o Mayar Natural Intense é perfeito para quem busca um perfume marcante com sensação de frescor luxuoso, combinando frutas exóticas, acordes aquáticos e flores delicadas em uma fragrância única e viciante.

Sua combinação refrescante de frutas tropicais, notas cítricas luminosas, coco cremoso, flores brancas e musk suave cria uma assinatura leve, sofisticada e extremamente agradável, perfeita para mulheres que gostam de perfumes elegantes e cheios de personalidade.

É aquele perfume que transmite liberdade, frescor e sofisticação, deixando uma sensação limpa e marcante por onde passa. ✨

🌊 Família Olfativa

Floral Aquático Frutado

✨ Notas em destaque

* Frutas tropicais refrescantes
* Limão e acordes cítricos vibrantes
* Coco cremoso sofisticado
* Flores brancas delicadas
* Musk limpo e confortável

💖 Ideal para

✔️ Uso diário
✔️ Dias quentes e clima tropical
✔️ Praia, viagens e momentos especiais
✔️ Mulheres que amam perfumes frescos e sofisticados
✔️ Quem busca fragrâncias elegantes e extremamente elogiadas

🛍️ Disponível em:

🌿 Frasco Original 100ml
💚 R$ 219,90

🌿 Decant 10ml
✨ R$ 39,90

🌿 Kit Descoberta
✨ 3 Decants de 10ml por R$ 109,90

Perfeito para conhecer a fragrância antes de investir no frasco completo ou experimentar diferentes perfumes da coleção.
🌿 Descubra por que o Mayar Natural Intense Lattafa se tornou um dos perfumes árabes mais desejados por mulheres que amam frescor, elegância e sofisticação.

Flor Árabe Perfumaria
Onde cada fragrância conta uma história. ✨`,
  },
  {
    id: "yara-lattafa",
    name: "Yara Lattafa",
    price: 199.90,
    originalPrice: 249.88,
    olfactoryFamily: "Floral Frutado Gourmand",
    gender: "Feminino",
    image: "/images/products/yara-lattafa.jpg",
    images: ["/images/products/yara-lattafa.jpg", "/images/products/yara-lattafa-tabela.jpeg"],
    description: `O perfume árabe feminino que conquistou mulheres no mundo inteiro.

Delicado, feminino e envolvente, o Yara é perfeito para quem busca uma fragrância elegante para o dia a dia. Sua combinação de notas florais, baunilha cremosa e musk cria um aroma confortável, sofisticado e extremamente agradável.

É aquele perfume que chama atenção de forma leve, recebe elogios e deixa sua marca por onde passa. ✨

🌷 Família Olfativa
Floral Frutado Gourmand

✨ Notas em destaque
* Flores delicadas
* Baunilha cremosa
* Musk aveludado
* Toques frutados suaves

💖 Ideal para
✔️ Uso diário
✔️ Trabalho
✔️ Passeios
✔️ Mulheres que amam perfumes doces e elegantes

🛍️ Disponível em:

🌸 Frasco Original 100ml
💗 R$ 199,90

🌸 Decant 10ml
✨ R$ 34,90

🌸 Kit Descoberta
✨ 3 Decants de 10ml por R$ 99,90

Perfeito para conhecer a fragrância antes de investir no frasco completo ou experimentar diferentes perfumes da coleção.`,
  },
  
  {
    id: "yara-lattafa-candy",
    name: "Yara Lattafa Candy",
    price: 199.90,
    originalPrice: 249.88,
    olfactoryFamily: "Floral Frutado Gourmand",
    gender: "Feminino",
    image: "/images/products/yara-lattafa-candy.jpg",
    images: ["/images/products/yara-lattafa-candy.jpg", "/images/products/yara-lattafa-candy-tabela.jpeg"],
    description:
      `O perfume árabe feminino que conquistou as apaixonadas por fragrâncias doces e irresistíveis.

Divertido, feminino e extremamente viciante, o Yara Candy é perfeito para quem ama perfumes adocicados, cremosos e cheios de personalidade.

Sua combinação de frutas suculentas, acordes açucarados, baunilha cremosa e musk cria uma fragrância moderna, envolvente e deliciosa, que chama atenção sem perder a elegância.
 
É aquele perfume que transmite alegria, feminilidade e deixa um rastro marcante por onde passa. ✨

🌷 Família Olfativa

Floral Frutado Gourmand

✨ Notas em destaque

* Frutas vermelhas suculentas
* Acordes doces e açucarados
* Baunilha cremosa
* Musk aveludado
* Toques gourmand irresistíveis

💖 Ideal para

✔️ Uso diário
✔️ Passeios
✔️ Encontros especiais
✔️ Mulheres que amam perfumes doces e modernos
✔️ Quem gosta de fragrâncias femininas que recebem elogios

🛍️ Disponível em:

🍬 Frasco Original 100ml
💗 R$ 199,90

🍬 Decant 10ml
✨ R$ 34,90

🍬 Kit Descoberta
✨ 3 Decants de 10ml por R$ 99,90

Perfeito para conhecer a fragrância antes de investir no frasco completo ou experimentar diferentes perfumes da coleção.
`,
  },
  {
    id: "yara-lattafa-moi",
    name: "Yara Lattafa Moi",
    price: 199.90,
    originalPrice: 249.88,
    olfactoryFamily: "Floral Gourmand Cremoso",
    gender: "Feminino",
    image: "/images/products/yara-lattafa-moi.jpg",
    images: ["/images/products/yara-lattafa-moi.jpg", "/images/products/yara-moi-lattafa-tabela.jpeg"],
    description:
      `O perfume árabe feminino que conquista pela elegância e cremosidade.

Delicado, sofisticado e envolvente, o Yara Moi é perfeito para quem busca uma fragrância feminina marcante, confortável e extremamente elegante.

Sua combinação de flores brancas, baunilha cremosa, notas gourmand e musk cria um aroma aveludado, luxuoso e irresistivelmente feminino.

É aquele perfume que transmite sofisticação sem esforço, recebe elogios e deixa uma impressão memorável por onde passa. ✨

🌷 Família Olfativa

Floral Gourmand Cremoso

✨ Notas em destaque

* Flores brancas delicadas
* Baunilha cremosa
* Caramelo suave
* Musk aveludado
* Acordes gourmand sofisticados

💖 Ideal para

✔️ Uso diário
✔️ Trabalho
✔️ Encontros especiais
✔️ Passeios
✔️ Mulheres que amam perfumes doces, elegantes e sofisticados

🛍️ Disponível em:

🤍 Frasco Original 100ml
✨ R$ 199,90

🤍 Decant 10ml
✨ R$ 34,90

🤍 Kit Descoberta
✨ 3 Decants de 10ml por R$ 99,90

Perfeito para conhecer a fragrância antes de investir no frasco completo ou experimentar diferentes perfumes da coleção.`,
  },
  {
    id: "haya-lattafa",
    name: "Haya Lattafa",
    price: 299.90,
    originalPrice: 374.88,
    olfactoryFamily: "Floral Frutado Sofisticado",
    gender: "Feminino",
    image: "/images/products/haya-lattafa.jpg",
    images: ["/images/products/haya-lattafa.jpg", "/images/products/Haya-lattafa-tabela.jpeg"],
    description: `A fragrância árabe que transmite delicadeza, elegância e feminilidade em cada detalhe.

Haya by Lattafa é um perfume sofisticado e envolvente, criado para quem ama fragrâncias elegantes, modernas e extremamente marcantes. Sua composição combina notas florais delicadas, acordes frutados vibrantes e um fundo cremoso sofisticado, criando uma experiência olfativa luxuosa e inesquecível. ✨

Com excelente fixação e projeção equilibrada, é perfeito para mulheres que desejam transmitir presença, charme e sofisticação em qualquer ocasião.

💎 Família Olfativa
Floral Frutado Sofisticado

✨ Notas em destaque

Notas de Topo
* Frutas Vermelhas
* Tangerina
* Champagne Rosé

Notas de Corpo
* Gardênia
* Orquídea Baunilha
* Jasmim

Notas de Fundo
* Sândalo
* Âmbar
* Musk

💖 Ideal para quem busca

✔️ Perfume feminino sofisticado e elegante
✔️ Excelente fixação e ótima performance
✔️ Uso diário ou ocasiões especiais
✔️ Fragrância extremamente elogiada
✔️ Presença delicada e marcante

🛍️ Disponível em

🌸 Frasco Original 100ml
✨ R$ 299,90

🌸 Decant 10ml
✨ R$ 44,90

🌸 Kit Descoberta
✨ 3 Decants de 10ml por R$ 119,90

Perfeito para conhecer a fragrância antes de investir no frasco completo ou experimentar novas experiências olfativas da coleção.`,
  },
  {
    id: "khamrah-qahwa-lattafa",
    name: "Khamrah Qahwa Lattafa",
    price: 320.0,
    originalPrice: 400.0,
    olfactoryFamily: "Oriental Especiado Gourmand",
    gender: "Unissex",
    image: "/images/products/khamrah-qahwa-lattafa.jpg",
    images: [
      "/images/products/khamrah-qahwa-lattafa.jpg",
      "/images/products/khamrah-qahwa-lattafa-tabela.jpeg",
    ],
    description: `O perfume árabe unissex que combina luxo, café intenso, sofisticação e uma assinatura olfativa absolutamente viciante.

O Khamrah Qahwa by Lattafa é uma versão ainda mais intensa e envolvente da linha Khamrah, trazendo uma combinação irresistível de café encorpado, especiarias quentes, acordes gourmand sofisticados e madeiras nobres, criando uma fragrância marcante, elegante e extremamente sedutora.

Seu aroma entrega uma experiência olfativa quente e sofisticada, perfeita para quem busca perfumes com personalidade forte, presença memorável e performance acima da média.

Com excelente projeção e fixação prolongada, é aquele perfume que chama atenção, transmite luxo e deixa rastro por onde passa. ✨

Ideal tanto para homens quanto mulheres que gostam de fragrâncias intensas e premium.

🔥 Família Olfativa

Oriental Especiado Gourmand

✨ Notas em Destaque
Notas de Topo

* Canela
* Cardamomo
* Gengibre

Notas de Corpo

* Café
* Pralinê
* Frutas Cristalizadas

Notas de Fundo

* Baunilha
* Fava Tonka
* Benjoim
* Musk
* Madeira Âmbar

☕ Ideal para quem busca

✔️ Perfumes intensos, sofisticados e extremamente marcantes
✔️ Fragrâncias com perfil quente e gourmand premium
✔️ Excelente projeção e fixação prolongada
✔️ Uso noturno e ocasiões especiais
✔️ Perfumes unissex elegantes e diferenciados
✔️ Aromas luxuosos com assinatura exclusiva

🛍️ Disponível em

🤎 Frasco Original 100ml
✨ R$ 320,00

🤎 Decant 10ml
✨ R$ 49,90

🤎 Kit Descoberta
✨ 3 Decants de 10ml por R$ 129,90

Perfeito para experimentar a fragrância antes de investir no frasco completo ou levar seu perfume favorito para qualquer lugar.`,
  },
  {
    id: "khamrah-lattafa",
    name: "Khamrah Lattafa",
    price: 220.0,
    originalPrice: 275.0,
    olfactoryFamily: "Oriental Especiado Gourmand",
    gender: "Unissex",
    image: "/images/products/khamrah-lattafa.jpg",
    images: [
      "/images/products/khamrah-lattafa.jpg",
      "/images/products/khamrah-lattafa-tabela.jpeg",
    ],
    description: `O perfume árabe que transmite luxo, intensidade e uma sofisticação absolutamente viciante.

O Khamrah by Lattafa é uma fragrância premium criada para pessoas que apreciam perfumes marcantes, quentes e extremamente envolventes. Com perfil unissex, ele conquista tanto homens quanto mulheres que buscam uma assinatura olfativa elegante, sensual e memorável.

Sua composição combina especiarias quentes, acordes gourmand sofisticados, baunilha cremosa, madeiras nobres e nuances licorosas, criando uma fragrância luxuosa, intensa e extremamente diferenciada.

Com excelente projeção e fixação prolongada, é aquele perfume que chama atenção, transmite presença e deixa um rastro elegante por onde passa. ✨

Perfeito para quem ama perfumes sofisticados, intensos e com performance acima da média.

🔥 Família Olfativa

Oriental Gourmand Ambarado

✨ Notas em destaque

Notas de Topo
* Canela
* Noz Moscada
* Bergamota

Notas de Corpo
* Pralinê
* Tâmaras
* Tuberosa
* Mahonial

Notas de Fundo
* Baunilha
* Fava Tonka
* Mirra
* Benjoim
* Madeira Âmbar

💎 Ideal para quem busca

✔️ Perfumes intensos e extremamente sofisticados
✔️ Fragrâncias unissex premium
✔️ Excelente projeção e fixação prolongada
✔️ Perfumes quentes, elegantes e marcantes
✔️ Uso noturno e ocasiões especiais
✔️ Aromas extremamente elogiados e diferenciados

🛍️ Disponível em

🥃 Frasco Original 100ml
✨ R$ 220,00

🥃 Decant 10ml
✨ R$ 39,90

🥃 Kit Descoberta
✨ 3 Decants de 10ml por R$ 109,90

Ideal para conhecer a fragrância antes de investir no frasco completo ou experimentar diferentes perfumes da coleção.`,
  },
];
